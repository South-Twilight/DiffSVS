...
class LatentDataModuleFromConfig(DataModuleFromConfig):
    ...
    def __init__(self, batch_size, num_workers, data_dir=None,
        ...
        global_dataset_cfg = {k: v for k, v in global_dataset_cfg.items() if v is not None}

        # 3. 动态合并参数 (安全更新 yaml 中的独立配置)
        for name, split in {'train': train, 'validation': validation, 'test': test}.items():
            if split is not None:
                # 统一传给 Dataset 的配置字典名为 dataset_cfg
                if not hasattr(split.params, 'dataset_cfg') or split.params.dataset_cfg is None:
                    split.params.dataset_cfg = global_dataset_cfg
                else:
                    split.params.dataset_cfg.update(global_dataset_cfg)
